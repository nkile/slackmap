...
#NOTICE: Ignore the above and look at ~/slacktee.conf for the configuration data for this script!
...
function send_message(){
	...
	json="{\"channel\": \"#$channel\", \"username\": \"$username\", \"text\": \"$escapedText\", \"icon_emoji\": \":$icon:\"}"
        ...
        post_result=`curl -X POST --data-urlencode "payload=$json" "https://$slack_domain.slack.com/services/$token" 2>/dev/null`
    fi
}
...
filename="$tmp_dir/$filetitle$$-$timestamp.html"
...
if [[ $mode == "buffering" ]]; then
    ...
    text="Nmap full scan $title has been uploaded:\n$download_url"
    send_message "$text"
    rm $filename
fi
